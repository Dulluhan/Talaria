...
b_w = 0.00001; % flywheel/motor friction
...
theta_p = 1/12*pi; %initial conditions

%% control group variables
theta_po = theta_p;
x_o = X;
B_o = B;
k = 0.9; %differential controller gain

%% Operation Simulation
while t < 100 && ishandle(figs)
    ...
    T_m = ((-b_p^2/I_p)-L_eq*m_eq*g)*theta_p; %small angle appx. 
    T_m = T_m + abs((theta_p+360)/180)*T_m;
    

    %-abs((theta_p+360)/180)*T_grav;
    %-abs((theta_p+360)/180)*(T_grav + b_w*X(2) + b_p*X(1)); 
    %%k*(2*pi-theta_p)/(2*pi)*T_grav + b_w*X(2) + b_p*X(1);
    
    disp([T_m,T_grav,b_w*X(2),b_p*X(1)]);
    %% Test group
    U = [T_m ; g]; %Input Vectord
    dX = A*X + B*U;
    X = dX.*dt + X;
    theta_p = X(1)*dt + theta_p; % integrates over time step %Displacement
    if theta_p > 2*pi
        theta_p = theta_p - 2*pi;
    elseif theta_p < - 2*pi
        theta_p = theta_p+2*pi;
...
%% Close graph at terminal 
function cleanupf(figs)
    close(figs);
end
